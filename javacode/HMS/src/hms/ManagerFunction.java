...
public class ManagerFunction  { //系统管理员功能类
    JFrame MangerJFrame =new JFrame("酒店管理系统-----[经理模式]");
    Container MangerCon = MangerJFrame.getContentPane();
    JPanel pn_function=new JPanel(); //放置各种功能页面的容器
    JLabel lb_topFunction=new JLabel(); //顶部信息栏，当前功能
    JLabel lb_tips=new JLabel(); //提示窗口的内容
    String Hsid;

    public ManagerFunction(String admin_num) {
        Hsid = admin_num;
        MangerJFrame.setSize(1300, 800);
        MangerJFrame.setLocationRelativeTo(null);
        MangerJFrame.setResizable(false);
        MangerJFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
        MangerJFrame.setLayout(null);

        // 初始化面板和布局
        MangerCon.setLayout(null);
        MangerJFrame.setContentPane(MangerCon);
        ...
        JButton bt_staffManagement = new JButton("员工管理");
        bt_staffManagement.setFont(new Font("黑体", 0, 20));
        bt_staffManagement.setContentAreaFilled(false);
        bt_staffManagement.setBounds(0, 325, 249, 50);
        bt_staffManagement.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                lb_topFunction.setText("[员工管理]");
                pn_function.removeAll();
                pn_function.add(staffManagement());
                pn_function.repaint();
                pn_function.validate();
            }
        });
        ...
        bt_serviceRecord.setBounds(0, 380, 249, 50);
        ...
        bt_reservationRecord.setBounds(0, 435, 249, 50);
        ...
        bt_feedbackRecord.setBounds(0, 490, 249, 50);
        ...
        bt_exit.setBounds(0, 545, 249, 50);
        ...
        pn_menu.add(bt_staffManagement);
        ...
        // 添加所有面板到容器
        MangerCon.add(pn_topUser);
        MangerCon.add(pn_topFunction);
        MangerCon.add(pn_menu);
        MangerCon.add(pn_function);
        pn_function.add(pn_welcome);

        // 设置可见
        MangerJFrame.setVisible(true);
    }
    ...
    public JPanel customerQuery() {
        ...
        // 选项卡面板
        JTabbedPane tabbedPane = new JTabbedPane();
        tabbedPane.setBounds(10, 10, 1020, 650); // 设置选项卡面板的大小和位置

        // 选项卡1：所有顾客
        JPanel allCustomersPanel = new JPanel(null);
        JTable allCustomersTable = new JTable();
        JScrollPane allCustomersScrollPane = new JScrollPane(allCustomersTable);
        allCustomersScrollPane.setBounds(10, 10, 1000, 600); // 设置滚动面板的大小和位置
        allCustomersPanel.add(allCustomersScrollPane);
        ...
        try {
            ...
            allCustomersTable.setModel(buildTableModel(rs));
            allCustomersTable.setFont(new Font("黑体", Font.PLAIN, 18)); // 设置表格字体
            allCustomersTable.setRowHeight(25); // 设置表格行高
            JTableHeader header = allCustomersTable.getTableHeader();
        ...
        customerScrollPane.setBounds(10, 70, 1000, 540); // 设置滚动面板的大小和位置
        ...
        // 添加选项卡到选项卡面板
        tabbedPane.addTab("所有顾客", allCustomersPanel);
        tabbedPane.addTab("查询顾客", searchCustomerPanel);
    ...
    public JPanel roomQuery() {
        ...
        // 选项卡1：所有房间
        JPanel allRoomsPanel = new JPanel(null);
        JTable allRoomsTable = new JTable(); // 创建用于显示所有房间信息的表格
        JScrollPane allRoomsScrollPane = new JScrollPane(allRoomsTable);
        allRoomsScrollPane.setBounds(10, 10, 1000, 600); // 设置滚动面板的大小和位置
        ...
        try {
            ...
            Statement stmt = conn.createStatement();
            ResultSet rs = stmt.executeQuery(query);
            allRoomsTable.setModel(buildTableModel(rs));
            allRoomsTable.setFont(new Font("黑体", Font.PLAIN, 18)); // 设置表格字体
            allRoomsTable.setRowHeight(25); // 设置表格行高
            JTableHeader header = allRoomsTable.getTableHeader();
            header.setFont(new Font("黑体", Font.BOLD, 20)); // 设置表头字体
            header.setPreferredSize(new Dimension(1, 30)); // 设置表头高度
        } catch (SQLException ex) {
            ex.printStackTrace();
        ...
        freeRoomsScrollPane.setBounds(10, 10, 1000, 600); // 设置滚动面板的大小和位置
        ...
        // 添加选项卡到选项卡面板
        tabbedPane.addTab("所有房间", allRoomsPanel);
        tabbedPane.addTab("空闲房间", freeRoomsPanel);
    ...
    public JPanel staffManagement() {
        ...
        // 选项卡1：解雇
        JPanel firePanel = new JPanel(null); // 使用 null 布局
        firePanel.setBounds(10,10,1020,300);// 设置面板大小

        // 解雇表单面板
        JPanel fireFormPanel = new JPanel(null); // 使用 null 布局
        fireFormPanel.setBounds(20, 20, 1000, 100); // 设置位置和大小
        JLabel fireLabel = new JLabel("员工ID:");
        fireLabel.setBounds(20, 20, 80, 30);
        JTextField fireField = new JTextField(15);
        fireField.setBounds(100, 20, 200, 30);
        JButton fireButton = new JButton("解雇");
        fireButton.setBounds(320, 20, 80, 30);

        fireFormPanel.add(fireLabel);
        fireFormPanel.add(fireField);
        fireFormPanel.add(fireButton);
        firePanel.add(fireFormPanel);

        // 解雇按钮事件
        fireButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String hsId = fireField.getText().trim();
                if (hsId.isEmpty()) {
                    lb_tips.setText("请输入员工ID！");
                    functionTips();
                    return;
                }
                try {
                    Connection conn = Home.connection;
                    String query = "DELETE FROM HotelStaff WHERE HSid = ?";
                    PreparedStatement pstmt = conn.prepareStatement(query);
                    pstmt.setString(1, hsId);
                    int rowsAffected = pstmt.executeUpdate();
                    if (rowsAffected > 0) {
                        lb_tips.setText("员工解雇成功！");
                    } else {
                        lb_tips.setText("未找到该员工！");
                    }
                    functionTips();
                } catch (SQLException ex) {
                    ex.printStackTrace();
        ...
        // 选项卡2：雇佣
        JPanel hirePanel = new JPanel(null); // 使用 null 布局
        hirePanel.setBounds(10,10,1020,300); // 设置面板大小

        // 雇佣表单面板
        JPanel hireFormPanel = new JPanel(null); // 使用 null 布局
        hireFormPanel.setBounds(20, 20, 1000, 250); // 设置位置和大小
        JLabel hireIdLabel = new JLabel("员工ID:");
        hireIdLabel.setBounds(20, 20, 80, 30);
        JTextField hireIdField = new JTextField(15);
        hireIdField.setBounds(100, 20, 200, 30);
        JLabel hireNameLabel = new JLabel("姓名:");
        hireNameLabel.setBounds(20, 60, 80, 30);
        JTextField hireNameField = new JTextField(15);
        hireNameField.setBounds(100, 60, 200, 30);
        JLabel hireGenderLabel = new JLabel("性别:");
        hireGenderLabel.setBounds(20, 100, 80, 30);
        JTextField hireGenderField = new JTextField(15);
        hireGenderField.setBounds(100, 100, 200, 30);
        JLabel hireAgeLabel = new JLabel("年龄:");
        hireAgeLabel.setBounds(20, 140, 80, 30);
        JTextField hireAgeField = new JTextField(15);
        hireAgeField.setBounds(100, 140, 200, 30);
        JLabel hirePhoneLabel = new JLabel("电话:");
        hirePhoneLabel.setBounds(320, 20, 80, 30);
        JTextField hirePhoneField = new JTextField(15);
        hirePhoneField.setBounds(400, 20, 200, 30);
        JLabel hireAddressLabel = new JLabel("地址:");
        hireAddressLabel.setBounds(320, 60, 80, 30);
        JTextField hireAddressField = new JTextField(15);
        hireAddressField.setBounds(400, 60, 200, 30);
        JLabel hirePasswordLabel = new JLabel("密码:");
        hirePasswordLabel.setBounds(320, 100, 80, 30);
        JTextField hirePasswordField = new JTextField(15);
        hirePasswordField.setBounds(400, 100, 200, 30);
        JLabel hirePositionLabel = new JLabel("职位:");
        hirePositionLabel.setBounds(320, 140, 80, 30);
        JTextField hirePositionField = new JTextField(15);
        hirePositionField.setBounds(400, 140, 200, 30);
        JLabel hirePermissionLabel = new JLabel("管理权限:");
        hirePermissionLabel.setBounds(20, 180, 80, 30);
        JTextField hirePermissionField = new JTextField(15);
        hirePermissionField.setBounds(100, 180, 200, 30);

        JButton hireButton = new JButton("雇佣");
        hireButton.setBounds(620, 180, 80, 30);

        hireFormPanel.add(hireIdLabel);
        hireFormPanel.add(hireIdField);
        hireFormPanel.add(hireNameLabel);
        hireFormPanel.add(hireNameField);
        hireFormPanel.add(hireGenderLabel);
        hireFormPanel.add(hireGenderField);
        hireFormPanel.add(hireAgeLabel);
        hireFormPanel.add(hireAgeField);
        hireFormPanel.add(hirePhoneLabel);
        hireFormPanel.add(hirePhoneField);
        hireFormPanel.add(hireAddressLabel);
        hireFormPanel.add(hireAddressField);
        hireFormPanel.add(hirePasswordLabel);
        hireFormPanel.add(hirePasswordField);
        hireFormPanel.add(hirePositionLabel);
        hireFormPanel.add(hirePositionField);
        hireFormPanel.add(hirePermissionLabel);
        hireFormPanel.add(hirePermissionField);
        hireFormPanel.add(hireButton);
        ...
        hireButton.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                String hsId = hireIdField.getText().trim();
                String name = hireNameField.getText().trim();
                String gender = hireGenderField.getText().trim();
                String age = hireAgeField.getText().trim();
                String phone = hirePhoneField.getText().trim();
                String address = hireAddressField.getText().trim();
                String password = hirePasswordField.getText().trim();
                String position = hirePositionField.getText().trim();
                String permission = hirePermissionField.getText().trim();
                if (hsId.isEmpty() || name.isEmpty() || gender.isEmpty() || age.isEmpty() ||
                        phone.isEmpty() || address.isEmpty() || password.isEmpty() || position.isEmpty() || permission.isEmpty()) {
                    lb_tips.setText("请填写所有信息！");
                    functionTips();
                    return;
                }
                try {
                    Connection conn = Home.connection;
                    String checkIdQuery = "SELECT COUNT(*) FROM HotelStaff WHERE HSid = ?";
                    PreparedStatement checkStmt = conn.prepareStatement(checkIdQuery);
                    checkStmt.setString(1, hsId);
                    ResultSet rs = checkStmt.executeQuery();
                    rs.next();
                    if (rs.getInt(1) > 0) {
                        lb_tips.setText("员工ID已存在！");
                        functionTips();
                        return;
                    }
                    String query = "INSERT INTO HotelStaff (HSid, HSname, HSgender, HSeage, HSphone, HSaddress, HSposition, HSpermission, HSpassword) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                    PreparedStatement pstmt = conn.prepareStatement(query);
                    pstmt.setString(1, hsId);
                    ...
                    pstmt.setInt(8, Integer.parseInt(permission));
                    pstmt.setString(9, password);
                    pstmt.executeUpdate();
                    lb_tips.setText("员工雇佣成功！");
                    functionTips();
                } catch (SQLException ex) {
                    ex.printStackTrace();
                    lb_tips.setText("数据库错误：" + ex.getMessage());
        ...
        // 添加选项卡到选项卡面板
        tabbedPane.addTab("解雇", firePanel);
        tabbedPane.addTab("雇佣", hirePanel);

        // 将选项卡面板添加到主面板
        tabbedPane.setBounds(0, 0, 1040, 300); // 设置选项卡面板位置和大小
        panel.add(tabbedPane);
    ...
    public void functionTips() { //操作时弹出的提示信息窗口
        JDialog tips=new JDialog(MangerJFrame,"  提示",true);
    ...
    public void choiceTips(String str) { //操作时弹出的提示选择窗口
        JDialog choiceTips=new JDialog(MangerJFrame,"  提示",true);
        JPanel pn_tips=new JPanel();
